...
function GuestBook({ fName }) {
  ...
  const handleSendMessage = useDebounce(() => {
    const isValid = validation({ name, wish,no, isAttend: willArrive });

    const now = new Date();
    const [day, month, year] = guestbookSection.time.split("/");
    const last = new Date(year, month - 1, day, 23, 59, 59);

    if (isValid && now.getTime() <= last.getTime()) {
      var data = { name, wish, no, isAttend: willArrive };
      fetch(WISH_API_LINK, {
        method: "POST",
        headers: {
